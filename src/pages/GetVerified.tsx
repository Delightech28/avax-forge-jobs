...
const tiers = [
	...
	},
	{
		...
		features: [
			...
			"AI mock interview & personalized tips (Coming Soon)",
...
const GetVerified = () => {
	...
	const getPlanType = () => {
		if (modalPlanIdx === 1 && modalPeriod === "monthly") return "ProMonthly";
		if (modalPlanIdx === 1 && modalPeriod === "annual") return "ProAnnual";
		if (modalPlanIdx === 2 && modalPeriod === "monthly") return "EliteMonthly";
		if (modalPlanIdx === 2 && modalPeriod === "annual") return "EliteAnnual";
		return null;
	};
		...
		<div className="min-h-screen bg-gradient-to-b from-[#0f172a] to-[#1e293b] text-white relative">
			...
			<main className="container mx-auto px-4 py-12 flex flex-col items-center">
				...
				<p className="text-lg text-slate-300 mb-8 text-center max-w-2xl">
					Enjoy a trusted experience, exclusive job access, and priority support. Stand out to employers and
				...
				<div className="flex items-center justify-center mb-6">
					<div className="flex bg-[#111827] rounded-full p-1 gap-1 border border-slate-700 shadow-inner">
						<button
							className={`px-5 py-1.5 rounded-full font-semibold transition-all duration-150 focus:outline-none focus:ring-2 focus:ring-red-400 shadow ${
								billingPeriod === "monthly"
									? "text-white bg-red-600"
									: "text-slate-300 bg-transparent hover:bg-slate-800"
							}`}
							style={{ zIndex: 1 }}
						...
						<div className="relative flex items-center">
							<button
								className={`px-5 py-1.5 rounded-full font-semibold transition-all duration-150 focus:outline-none focus:ring-2 focus:ring-red-400 ${
							...
							<span className="absolute -top-4 left-1/2 -translate-x-1/2 bg-green-600 text-xs px-2 py-0.5 rounded-full font-semibold shadow text-white whitespace-nowrap">
								Best Value
							</span>
						</div>
					</div>
				...
				{showModal && modalPlanIdx !== null && (
					...
					>
						...
						>
							<button
								className="absolute top-2 right-4 text-slate-400 hover:text-white text-xl"
								onClick={() => setShowModal(false)}
							>
								&times;
							</button>
							...
							<button
								...
								onClick={async () => {
									...
									const success = await subscribe(planType);
									if (success) {
